...
export function TeamActivityChronicle({ activities }: TeamActivityChronicleProps) {
  return (
    <div className="p-4 rounded-xs border-2 border-[#3B2415] bg-[#1A120C] shadow-2xl space-y-3">
      <div className="flex items-center justify-between border-b border-[#4A2C1D] pb-2">
        ...
        <span className="text-[9px] font-serif italic text-[#D7B05C]/60">Council Chronicle</span>
      </div>

      <div className="space-y-3 pt-1">
        {activities.map((act) => (
          <div key={act.id} className="text-xs font-sans space-y-1 border-b border-[#4A2C1D]/40 pb-2.5 last:border-0 last:pb-0">
            <p className="text-[#F8EEDB] leading-tight">
              <span className="font-extrabold text-[#D7B05C]">{act.user}</span> {act.action}
            </p>
