...
export function TeamDirectoryTable({ members, onSelectMember }: TeamDirectoryTableProps) {
  if (members.length === 0) {
    return (
      <div className="p-8 rounded-xs border-2 border-dashed border-[#8F6236]/50 bg-[#15100C] text-center space-y-3">
        ...
        <h3 className="font-serif font-black text-lg text-[#F8EEDB]">No Team Members Found</h3>
        <p className="text-xs font-sans text-[#D7B05C]/70 max-w-md mx-auto">
          No team members have joined this workspace. Invite collaborators to begin managing projects together.
        </p>
        <p className="text-[11px] font-serif italic text-[#D7B05C]/50">The Roundtable awaits its first officers.</p>
      </div>
    )
  }
            ...
            {members.map((member) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="p-3.5 pl-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="block text-[10px] text-[#D7B05C]/70 italic">{member.email}</span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="p-3.5">
                    <div>
                      <span className={`inline-block font-sans text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-xs border ${theme.badgeBg} ${theme.badgeBorder} ${theme.badgeText}`}>
                        {member.role}
                      </span>
                  ...
                  {/* Status */}
                  <td className="p-3.5">
                    <span className={`inline-flex items-center px-2 py-0.5 text-[9px] font-black uppercase tracking-wider rounded-xs border ${statusBadge}`}>
                      {member.status}
                    </span>
                  </td>
