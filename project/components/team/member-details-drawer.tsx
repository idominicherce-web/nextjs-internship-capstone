...
import { Calendar, Mail, Scroll, Trash2, X } from "lucide-react";
import { getDisciplineTheme } from "@/lib/roles";
import type { Member } from "./team-directory-table";
			...
			<div className="w-full max-w-md bg-[#1A120C] border-l-4 border-[#3B2415] h-full shadow-2xl p-6 overflow-y-auto space-y-6 relative flex flex-col justify-between">
				<div className="space-y-6">
					...
					<div className="flex items-start justify-between border-b border-[#4A2C1D] pb-4">
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<span
									className={`inline-block text-[9px] font-sans font-black uppercase tracking-wider px-2 py-0.5 rounded-xs border ${theme.badgeBg} ${theme.badgeBorder} ${theme.badgeText}`}
								>
									{member.role}
								</span>
							</div>
					...
					<div className="space-y-4 text-xs font-sans">
						<div>
							<span className="block text-[10px] font-black uppercase text-[#D7B05C]/70 tracking-wider">
								Royal Title
							</span>
							<p className="font-serif italic text-[#F8EEDB] text-sm">
								{theme.royalTitle}
							</p>
						</div>
						...
						<div>
							...
							<p className="flex items-center gap-1.5 text-[#F8EEDB] mt-0.5">
								...
								{member.projectCount} Active Projects
							</p>
						</div>
