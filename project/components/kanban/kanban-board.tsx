...
import { TaskCardData } from "@/components/kanban/task-card"
import { TaskDetailModal } from "@/components/modals/task-detail-modal"
import { KanbanColumn, List } from "@/components/kanban/kanban-column"
...
type OptimisticAction =
  | { type: "MOVE_TASK"; payload: { taskId: string; sourceListId: string; targetListId: string } }
...
export function KanbanBoard({ projectId, initialLists = [] }: KanbanBoardProps) {
        ...
        case "MOVE_TASK": {
          ...
          return newLists.map((list) => {
            if (list.id === targetListId) {
              return { ...list, tasks: [...list.tasks, { ...movedTask!, listId: targetListId }] }
            }
            return list
          })
        }
  ...
  const handleAddTask = async (listId: string) => {
    ...
    startTransition(async () => {
      setOptimisticLists({ type: "ADD_TASK", payload: { listId, newTask: tempTask } })
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    startTransition(() => {
      setOptimisticLists({
        type: "MOVE_TASK",
        payload: { taskId: activeTaskId, sourceListId: sourceList.id, targetListId: targetList.id },
      })
    })
  }
          ...
          {optimisticLists.length === 0 ? (
            <div className="text-center py-16 border-2 border-dashed border-[#8F6236]/40 bg-[#15100C] rounded-xs space-y-2">
              ...
              <p className="text-xs font-sans text-[#D7B05C]/70 max-w-sm mx-auto">
                Type a column name above to start organizing your project tasks on the board.
              </p>
            </div>
          ) : (
