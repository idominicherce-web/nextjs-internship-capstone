...
export function ActivityArchive({ activities }: ActivityArchiveProps) {
	...
	const getActionConfig = (action: string) => {
		...
		if (act.includes("update") || act.includes("edit")) {
			return {
				icon: Edit2,
				color: "text-amber-300 bg-amber-950/80 border-amber-600/60",
			};
		}
		...
		if (act.includes("delete") || act.includes("remove")) {
			return {
				icon: Trash2,
				color: "text-rose-400 bg-rose-950/80 border-rose-600/60",
			};
		}
		return {
			icon: Bookmark,
			color: "text-[#D7B05C] bg-[#2D1B10] border-[#8F6236]",
		};
	};
	...
	const getEntityIcon = (entityType?: string | null) => {
		...
		if (type.includes("project"))
			return <Folder size={12} className="text-[#D7B05C]" />;
		...
		<DashboardSection className="">
			...
			{activities.length === 0 ? (
				<div className="p-8 border-2 border-dashed border-[#8F6236]/40 bg-[#15100C] text-center rounded-xs space-y-2">
					...
					<p className="font-serif font-bold text-sm text-[#F8EEDB]">
						Royal Archive Empty
					</p>
					<p className="text-xs font-sans text-[#D7B05C]/70">
						No recent activity has been recorded. Actions performed within your
						workspace will be preserved here.
					</p>
				</div>
			) : (
