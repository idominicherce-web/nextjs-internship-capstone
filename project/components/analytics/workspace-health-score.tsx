...
export function WorkspaceHealthScore({
  ...
  const overduePenalty = totalTasks > 0 ? Math.min((overdueTasks / totalTasks) * 100, 100) : 0
  ...
  const healthScore = Math.round(
    completionComponent + overdueComponent + workloadComponent + projectComponent
  )

  const healthRating =
    healthScore >= 85
      ? { label: "Excellent", color: "text-emerald-400", border: "border-emerald-600" }
      : healthScore >= 70
      ? { label: "Operational", color: "text-amber-400", border: "border-amber-600" }
      : { label: "Attention Required", color: "text-rose-400", border: "border-rose-600" }
    ...
    <div className="relative rounded-xs border-4 border-[#3B2415] bg-gradient-to-b from-[#2D1B10] via-[#1A120C] to-[#100A07] p-6 shadow-2xl overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
        ...
        <div className="lg:col-span-4 flex flex-col items-center justify-center p-6 bg-[#15100C]/80 border-2 border-[#4A2C1D] rounded-xs shadow-inner text-center space-y-3">
          ...
          <div className={`relative flex items-center justify-center w-28 h-28 rounded-full border-4 ${healthRating.border} bg-[#100A07] shadow-[0_0_20px_rgba(0,0,0,0.8)]`}>
            <div className="text-center">
              <span className={`text-3xl font-black font-serif ${healthRating.color}`}>
                {healthScore}
              </span>
          ...
          <span className={`text-xs font-sans font-black uppercase tracking-wider px-3 py-1 rounded-xs bg-[#2D1B10] border ${healthRating.border} ${healthRating.color}`}>
            {healthRating.label}
          </span>
        </div>
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="border-b border-[#4A2C1D] pb-2">
            ...
            <p className="text-xs font-sans text-[#D7B05C]/70 italic">
              Live status report derived from active campaigns and task velocity.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 text-xs font-sans">
            ...
            <div className="p-3 bg-[#15100C]/60 border border-[#4A2C1D] rounded-xs space-y-1">
              <div className="flex items-center gap-2 text-[#F8EEDB] font-bold">
                <AlertTriangle
                  size={14}
                  className={overdueTasks > 0 ? "text-rose-400" : "text-emerald-400"}
                />
                <span>
                  {overdueTasks > 0 ? "Attention Required" : "No Critical Overdue Tasks"}
                </span>
              </div>
              <p className="text-[11px] text-[#D7B05C]/70">
                {overdueTasks > 0
            ...
            <div className="p-3 bg-[#15100C]/60 border border-[#4A2C1D] rounded-xs space-y-1">
              ...
              <p className="text-[11px] text-[#D7B05C]/70">
                {activeTasks} active task{activeTasks === 1 ? "" : "s"} currently assigned across workspaces.
              </p>
            </div>
          </div>
        </div>
