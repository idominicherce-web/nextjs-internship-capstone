...
import { User, CheckCircle2, Shield, Calendar, Monitor, Globe } from "lucide-react"
...
export function ProfileSettingsForm({ user }: ProfileSettingsFormProps) {
  const [fullName, setFullName] = useState(user.name || "Dominic Herce")
  const [email, setEmail] = useState(user.email || "dominic@example.com")
      ...
      <div className="rounded-xs border-2 border-[#3B2415] bg-[#1A120C] shadow-2xl p-6 sm:p-8 space-y-6">
        ...
        <div className="border-b border-[#4A2C1D] pb-4 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-xs font-sans text-[#D7B05C]/80 mt-1 italic">
              Update your personal information, contact email, and workspace role.
            </p>
          </div>
          ...
          <div className="min-w-0 flex-1 space-y-0.5">
            ...
            <div className="flex flex-wrap items-center gap-3 text-[11px] font-sans text-[#D7B05C]/80">
              ...
              <span className="flex items-center gap-1 italic text-[#D7B05C]/60">
                ...
                Member since {user.createdAt ? new Date(user.createdAt).toLocaleDateString("en-US", { month: "short", year: "numeric" }) : "Jan 2025"}
              </span>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="max-w-2xl space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            >
              <option value="Workspace Owner">Workspace Owner (Royal Sovereign)</option>
              <option value="Administrator">Administrator (Chancellor)</option>
              <option value="Project Manager">Project Manager (High Commander)</option>
              <option value="Developer">Developer (Royal Engineer)</option>
              <option value="Designer">Designer (Master Artisan)</option>
              <option value="QA Engineer">QA Engineer (Royal Inquisitor)</option>
            </select>
          </div>
          ...
          <div className="flex items-center justify-between pt-6 border-t border-[#4A2C1D]">
            ...
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={() => {
                  setFullName(user.name || "Dominic Herce")
                  setEmail(user.email || "dominic@example.com")
                }}
                className="px-4 py-2 border border-[#8F6236]/60 bg-[#15100C] text-[#D7B05C]/80 hover:text-white hover:border-[#8F6236] text-xs font-sans font-bold uppercase rounded-xs transition-colors cursor-pointer"
              >
                Cancel
              </button>
              ...
              <button
                type="submit"
                className="px-5 py-2 border-2 border-[#D7B05C] bg-gradient-to-b from-[#5B3922] via-[#3B2415] to-[#1A120C] text-[#FFF5D6] font-sans text-xs font-black uppercase tracking-wider rounded-xs shadow-md hover:border-[#FFF5D6] hover:shadow-[0_0_15px_rgba(215,176,92,0.3)] transition-all cursor-pointer"
              >
                Save Changes
              </button>
            </div>
